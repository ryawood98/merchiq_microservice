...
def adblock():
    ...
    for n_clusters in range(1,6):
        ...
        n_clusters_sums[n_clusters-1] = sum([abs(X[clusters==i] - X[clusters==i].mean()).mean() for i in range(clustering.n_clusters_)])

    # determine best number of clusters by finding last n_clusters with big enough reduction in mean distance
    min_mean_distance = 1
    n_clusters = np.where((n_clusters_sums[:-1] - n_clusters_sums[1:])>min_mean_distance)[0][-1]+2
...
def prediction():
    ...
    # print('data', data)
    if not request.args.get('min_price') or not request.args.get('max_price') or not request.args.get('item_names') or not request.args.get('retailer'):
    ...
    ### query database
    with engine.begin() as conn:
        df = pd.read_sql(sqlalchemy.text('SELECT * FROM promos WHERE item_name IN :item_names AND non_promo_price BETWEEN :min_price AND :max_price;'),
                                conn, params={'item_names':tuple(item_names), 'min_price':min_price, 'max_price':max_price}, parse_dates=['retailer_week'])
    ...
    min_date = df['retailer_week'].min()-timedelta(days=7*(df['retailer_week'].min()-(dt.now()-timedelta(days=7*104))).days//7)
    max_date = df['retailer_week'].min()+timedelta(days=7*(((dt.now()+timedelta(days=7*52))-df['retailer_week'].min()).days//7))
    calendar = pd.DataFrame(columns=['retailer_week'], data=np.arange(min_date, max_date, timedelta(days=7)))
    ...
    merch_grid_raw = df.loc[(df['non_promo_price'].between(min_price,max_price))].groupby(by=['retailer_week'])[[mechanic_col]].first().dropna()
    merch_grid_raw = calendar.merge(merch_grid_raw, on='retailer_week', how='left')
    merch_grid_raw[mechanic_col] = merch_grid_raw[mechanic_col].fillna(0.).round(2)
    merch_grid = merch_grid_raw.pivot_table(index='week', columns='year', values=mechanic_col, aggfunc=lambda x:next(iter(x.dropna()),None), dropna=False)

    # add latest year's actual dates for reference
    min_date_latest_year = calendar.loc[(calendar['year']==calendar['year'].max()),'retailer_week'].min()
    latest_year_dates = pd.DataFrame(index=np.arange(52), data={'retailer_week':np.arange(min_date_latest_year, min_date_latest_year+timedelta(days=7*52), timedelta(days=7))})
    ...
    # create lagged promo
    merch_grid_raw[mechanic_col+'_ly'] = merch_grid_raw[mechanic_col].shift(52).fillna(0.)

    # create smoothed ly promo
    merch_grid_raw[mechanic_col+'_smoothed'] = merch_grid_raw[mechanic_col+'_ly'].rolling(window=3, center=True, win_type='gaussian', min_periods=1).mean(std=1)
    merch_grid_raw[mechanic_col+'_predicted'] = merch_grid_raw[mechanic_col+'_smoothed'].shift(52)


    return jsonify({
        'status': 'good request',
        'data': {'historical':
                    {'week': merch_grid_raw.loc[(merch_grid_raw['retailer_week']<dt.now()), 'retailer_week'].to_list(),
                     'discount':merch_grid_raw.loc[(merch_grid_raw['retailer_week']<dt.now()), mechanic_col].to_list()},
                 'prediction':
                    {'week': merch_grid_raw.loc[(merch_grid_raw['retailer_week']>=dt.now()), 'retailer_week'].to_list(),
                     'discount':merch_grid_raw.loc[(merch_grid_raw['retailer_week']>=dt.now()), mechanic_col+'_predicted'].to_list()}
                }
        }), 200
