...
def prediction():
    ...
    merch_grid['tpr_disc_smoothed'] = merch_grid['tpr_disc_ly'].rolling(window=3,center=True,win_type="gaussian",min_periods=1).mean(std=1)
    ...
    historical_disc_tpr = merch_grid.loc[(merch_grid["retailer_week"] < dt.now()),'tpr_disc'].to_list()
    historical_disc_tpr_desc = merch_grid.loc[(merch_grid["retailer_week"] < dt.now()),'tpr_disc_desc'].to_list()
    prediction_disc_tpr = merch_grid.loc[(merch_grid["retailer_week"] >= dt.now()),'tpr_disc_predicted'].to_list()
    prediction_disc_tpr_desc = merch_grid.loc[(merch_grid["retailer_week"] >= dt.now()),'tpr_disc_desc_predicted'].to_list()
    ...
    def quantify_crl(s):
        ...
        if pd.notnull(s['quantity_threshold']):
            if s['quantity_threshold']>1:
                ...
                if pd.notnull(s['reward_total_price']):
                    if pd.notnull(s['non_promo_price_est']):
                        crl_disc = s['non_promo_price'] - s['reward_total_price']/s['quantity_threshold']
                        return pd.Series({'crl_disc': crl_disc, 'crl_disc_desc': 'Buy '+str(int(s['quantity_threshold']))+' Save $'+'{:.2f}'.format(crl_disc)+' Each'})
                    else:
                ...
                # BOGO
                    ...
                    if pd.notnull(s['non_promo_price_est']):
                        crl_disc = s['non_promo_price_est'] * s['reward_percent'] / s['quantity_threshold']
                        return pd.Series({'crl_disc': crl_disc, 'crl_disc_desc': 'Buy '+str(int(s['quantity_threshold']))+' Save '+str(int(s['reward_percent']))+'% of One'})
                    else:
                        return pd.Series({'crl_disc': 0., 'crl_disc_desc': '$0.00'})
            else:
                return pd.Series({'crl_disc': 0., 'crl_disc_desc': '$0.00'})
            ...
            if pd.notnull(s['reward_dollars']):
                if pd.notnull(s['non_promo_price_est']):
                    crl_disc = s['reward_dollars'] / (s['spend_threshold'] / s['non_promo_price_est'])
                    return pd.Series({'crl_disc': crl_disc, 'crl_disc_desc': 'Spend $'+'{:.2f}'.format(s['spend_threshold'])+' Save $'+'{:.2f}'.format(s['reward_dollars'])})
                else:
                    crl_disc = s['reward_dollars']
                    return pd.Series({'crl_disc': crl_disc, 'crl_disc_desc': '$'+'{:.2f}'.format(crl_disc)})
            else:
                return pd.Series({'crl_disc': 0., 'crl_disc_desc': '$0.00'})
    ...
    # Create smoothed ly promo
    merch_grid['crl_disc_smoothed'] = merch_grid['crl_disc_ly'].rolling(window=3,center=True,win_type="gaussian",min_periods=1).mean(std=1)
    ...
    # Create lists to send to client
    historical_disc_crl = merch_grid.loc[(merch_grid["retailer_week"] < dt.now()),'crl_disc'].to_list()
    historical_disc_crl_desc = merch_grid.loc[(merch_grid["retailer_week"] < dt.now()),'crl_disc_desc'].to_list()
    prediction_disc_crl = merch_grid.loc[(merch_grid["retailer_week"] >= dt.now()),'crl_disc_predicted',].to_list()
    prediction_disc_crl_desc = merch_grid.loc[(merch_grid["retailer_week"] >= dt.now()),'crl_disc_desc_predicted',].to_list()
    ...
    def quantify_coupon(s):
        ...
        if pd.notnull(s['coupon_quantity_threshold']):
            ...
            if pd.notnull(s['coupon_dollar_value']):
                ...
                return pd.Series({'coupon_disc': coupon_disc, 'coupon_disc_desc': '$'+'{:.2f}'.format(s['coupon_dollar_value'])+' Off '+str(int(s['coupon_quantity_threshold']))})
    ...
    # Create smoothed ly promo
    merch_grid['coupon_disc_smoothed'] = merch_grid['coupon_disc_ly'].rolling(window=3,center=True,win_type="gaussian",min_periods=1).mean(std=1)
    ...
    # Create lists to send to client
    historical_disc_coupon = merch_grid.loc[(merch_grid["retailer_week"] < dt.now()),'coupon_disc'].to_list()
    historical_disc_coupon_desc = merch_grid.loc[(merch_grid["retailer_week"] < dt.now()),'coupon_disc_desc'].to_list()
    prediction_disc_coupon = merch_grid.loc[(merch_grid["retailer_week"] >= dt.now()),'coupon_disc_predicted'].to_list()
    prediction_disc_coupon_desc = merch_grid.loc[(merch_grid["retailer_week"] >= dt.now()),'coupon_disc_desc_predicted'].to_list()

    data_pred = DataPred(
        historical_tpr = DataSeries(week=historical_week, discount=historical_disc_tpr, label=historical_disc_tpr_desc),
        prediction_tpr = DataSeries(week=prediction_week, discount=prediction_disc_tpr, label=prediction_disc_tpr_desc),

        historical_crl = DataSeries(week=historical_week, discount=historical_disc_crl, label=historical_disc_crl_desc),
        prediction_crl = DataSeries(week=prediction_week, discount=prediction_disc_crl, label=prediction_disc_crl_desc),
